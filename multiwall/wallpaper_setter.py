...
def get_wallpaper_path(timestamp=None):
    ...
    Args:
        timestamp: Optional timestamp string to make filename unique
    """
    config_dir = Path.home() / ".config" / "multiwall"
    config_dir.mkdir(parents=True, exist_ok=True)
    
    if timestamp:
        # Clean up old wallpapers first
        try:
            for old_file in config_dir.glob("current_wallpaper_*.jpg"):
                try:
                    old_file.unlink()
                    logger.debug(f"Removed old wallpaper: {old_file}")
                except Exception as e:
                    logger.warning(f"Could not remove old wallpaper {old_file}: {e}")
        except Exception as e:
            logger.warning(f"Error cleaning up old wallpapers: {e}")
            
        filename = f"current_wallpaper_{timestamp}.jpg"
    else:
        filename = "current_wallpaper.jpg"
        
    wallpaper_path = str(config_dir / filename)
...
def test_gsettings_access():
    ...
    try:
        ...
        result = subprocess.run(
            ...
            text=True,
            timeout=5,
            cwd='/' if is_running_in_flatpak() else None
        )
...
def apply_wallpaper_native(image_path, dark_mode=False):
    """
    Apply wallpaper using gsettings (native method).
    
    Args:
        image_path: Full path to wallpaper image
        dark_mode: If True, only set dark mode wallpaper. If False, set both.
        
    Returns:
        tuple: (success: bool, message: str)
    """
    logger.info(f"Attempting to apply wallpaper with native gsettings (dark_mode={dark_mode})...")
    
    try:
        # Test read access first
        # Test read access first
        success, error_msg = test_gsettings_access()
        if not success:
            return False, f"Cannot access gsettings: {error_msg}"
        
        picture_uri = f'file://{image_path}'
        logger.debug(f"Picture URI: {picture_uri}")
        
        base_cmd = ['gsettings', 'set', 'org.gnome.desktop.background']
        if is_running_in_flatpak():
            base_cmd = ['flatpak-spawn', '--host'] + base_cmd
            
        if dark_mode:
            # Only set dark mode
            logger.debug("Setting picture-uri-dark only...")
            result = subprocess.run(
                base_cmd + ['picture-uri-dark', picture_uri],
                capture_output=True, text=True, timeout=10,
                cwd='/' if is_running_in_flatpak() else None
            )
            
            if result.returncode != 0:
                logger.error(f"Failed to set picture-uri-dark: {result.stderr}")
                return False, f"Error setting picture-uri-dark: {result.stderr}"
            logger.debug("picture-uri-dark set successfully")
        else:
            # Set both light and dark mode
            logger.debug("Setting picture-uri (light mode)...")
            result = subprocess.run(
                base_cmd + ['picture-uri', picture_uri],
                capture_output=True, text=True, timeout=10,
                cwd='/' if is_running_in_flatpak() else None
            )
        ...
        result = subprocess.run(
            base_cmd + ['picture-options', 'spanned'],
            capture_output=True, text=True, timeout=10,
            cwd='/' if is_running_in_flatpak() else None
        )
        ...
        verify_cmd = ['gsettings', 'get', 'org.gnome.desktop.background']
        if is_running_in_flatpak():
            verify_cmd = ['flatpak-spawn', '--host'] + verify_cmd
            
        result = subprocess.run(
            verify_cmd + ['picture-uri-dark' if dark_mode else 'picture-uri'],
        ...
        mode_text = "dark mode only" if dark_mode else "both light and dark modes"
        logger.info(f"Wallpaper applied successfully with gsettings ({mode_text})")
        return True, f"Wallpaper applied successfully with gsettings ({mode_text})"
...
def create_manual_instructions(image_path, dark_mode=False):
    """
    Create a shell script with manual instructions for applying wallpaper.
    
    Args:
        image_path: Path to wallpaper image
        dark_mode: If True, only set dark mode wallpaper
...
# Script to apply MultiWall wallpaper (dark mode only)

echo "Applying wallpaper to dark mode..."
gsettings set org.gnome.desktop.background picture-uri-dark 'file://{image_path}'
gsettings set org.gnome.desktop.background picture-options 'spanned'
echo "✅ Dark mode wallpaper applied successfully"
echo "Image: {image_path}"
"""
    else:
        script_content = f"""#!/bin/bash
# Script to apply MultiWall wallpaper (light and dark modes)

echo "Applying wallpaper..."
gsettings set org.gnome.desktop.background picture-uri 'file://{image_path}'
gsettings set org.gnome.desktop.background picture-uri-dark 'file://{image_path}'
gsettings set org.gnome.desktop.background picture-options 'spanned'
echo "✅ Wallpaper applied successfully to both modes"
echo "Image: {image_path}"
"""
...
def apply_wallpaper(image_path, dark_mode=False):
    """
    ...
    logger.info(f"Dark mode only: {dark_mode}")
    ...
    if is_running_in_flatpak():
        ...
        success, message = apply_wallpaper_native(final_path, dark_mode)
        if success:
        ...
        script_path = create_manual_instructions(final_path, dark_mode)
        mode_text = "dark mode" if dark_mode else "both light and dark modes"
        manual_msg = (
            f"⚠️ Could not apply automatically from Flatpak.\n"
            f"Error: {message}\n\n"
            f"Wallpaper was generated at:\n{final_path}\n\n"
            f"To apply manually ({mode_text}):\n"
    ...
    # If running in Docker or native
        ...
        success, message = apply_wallpaper_native(image_path, dark_mode)
        if success:
            logger.info("Wallpaper applied successfully")
            return True, message, None
        
        # Create fallback script
        script_path = create_manual_instructions(image_path, dark_mode)
        mode_text = "dark mode" if dark_mode else "both light and dark modes"
        fallback_msg = (
            f"⚠️ gsettings failed: {message}\n\n"
            f"To apply the wallpaper ({mode_text}), run:\n"
            f"bash {script_path}"
        )
