...
def apply_mode_to_image(img, target_size, mode, bgcolor):
    tw, th = target_size
    print(f"Aplicando modo '{mode}' a imagen {img.size} -> {target_size}")
...
def compose_image(monitors, states, scale_preview=None):
    ...
    for m in monitors:
        ...
        print(f"Monitor geometría: {geom.x}, {geom.y}, {geom.width}x{geom.height}")  # Debug
    ...
    print(f"Canvas total: {total_w}x{total_h}")  # Debug
    ...
    for i, (x, y, w, h) in enumerate(norm):
        ...
        if file and os.path.exists(file):
            ...
            print(f"Monitor {i}: Imagen cargada desde {file}")  # Debug
        ...
        if img:
            ...
            print(f"Monitor {i}: Usando color de fondo {bcolor}")  # Debug
